...
**Your agent has an address.** A real account gets its own, created on first use, and mail arriving at it is verified and deduplicated by event id, then routed to the claim it belongs to. Guests share one address instead, and that is deliberate: the plan allows three inboxes, and provisioning one per visitor spent that allowance within the first few people to open the deployed app, so whoever arrived after the last slot got an error where an address should be. A shared address cannot honestly be attributed to one of many guests, so guest mail is not routed onto anyone's ledger and the panel says so rather than pretending. One of the three inboxes is that shared guest address, so two accounts can hold one of their own; past that the app returns a sentence explaining why rather than an error trace, and the guest button still shows the whole product. The agent never asks for a password to your real mailbox and never reads your life. It reads what you forward.
